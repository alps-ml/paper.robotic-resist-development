...
class Meca_Control(Node):
    def __init__(self):
        ...
        self.joint_subscriber_robot1_ = self.create_subscription(JointState, "/joint_states",
                                                                 self.update_robot1_joints_callback, 10)
        self.pose_subscriber_robot1_ = self.create_subscription(Pose, f"{ROBOT1['namespace']}/MecademicRobot_pose_fb",
        ...
        self.chip_pose_subscriber_ = self.create_subscription(
            PoseStamped,
            '/chip_pose_calculator/detected_chip_world_pose', # Make sure this matches the publisher topic
            self.chip_pose_callback,
            10)
        self.latest_chip_pose_ = None # Variable to store the received pose
        self.get_logger().info("Subscribed to /chip_pose_calculator/detected_chip_world_pose")
    ...
    def run(self):
        ...
        pose_reached = self.move_pose_and_wait(ROBOT1['namespace'], pose[0], pose[1], pose[2], pose[3], pose[4], pose[5])
        if pose_reached:
        ...
        self.get_logger().info("Waiting for the first chip pose to be detected...")
        while self.latest_chip_pose_ is None and rclpy.ok():
            rclpy.spin_once(self, timeout_sec=0.1)
            time.sleep(0.1)

        if not rclpy.ok() or self.latest_chip_pose_ is None:
            self.get_logger().error("No chip pose received or node shutting down.")
            return
        
        chip_pose_msg = self.latest_chip_pose_.pose

        # Get the chip's yaw for use in both calculations
        q_chip = chip_pose_msg.orientation
        ...
        P_tcp_target_m = np.array([
            chip_pose_msg.position.x,
            chip_pose_msg.position.y,
            0.100  # Target Z for the TCP is 100mm, converted to meters
        ])
        ...
        #chip_yaw_deg = (chip_yaw_deg + 180.0) % 360.0 - 180.0
        ...
        command_pose = [
            ...
            95.0,                          # Use hardcoded Z of 100mm
            manual_alpha,                   # Use your manual Alpha
            manual_beta,                    # Use your manual Beta
            manual_gamma                    # Use your manual Gamma
        ]

        self.get_logger().info(f"Commanding FLANGE to P(mm): ({command_pose[0]:.2f}, {command_pose[1]:.2f}, {command_pose[2]:.2f}), "
        ...
        # --- 3. Send the Command ---
        if self.move_pose_and_wait(ROBOT1['namespace'], *command_pose):
        ...
        '''    
        input("Press Enter to move the robot to this pose...")
        chip_pose_in_base_msg = self.latest_chip_pose_.pose
        self.get_logger().info(f"Successfully got chip pose in frame 'meca_base_link'.")
        self.get_logger().info(f"Chip pose: {chip_pose_in_base_msg.position.x}, {chip_pose_in_base_msg.position.y}, {chip_pose_in_base_msg.position.z}, {chip_pose_in_base_msg.orientation.x}, {chip_pose_in_base_msg.orientation.y}, {chip_pose_in_base_msg.orientation.z}, {chip_pose_in_base_msg.orientation.w}")

        # --- STEP 1: Determine the target orientation for the FLANGE using YOUR formula ---
        
        # First, get the chip's world yaw from its detected Z-up orientation
        q_chip = chip_pose_in_base_msg.orientation
        r_chip_Zup = R.from_quat([q_chip.x, q_chip.y, q_chip.z, q_chip.w])
        chip_yaw_deg = r_chip_Zup.as_euler('zyx', degrees=True)[0]
        self.get_logger().info(f"Detected Chip Yaw: {chip_yaw_deg:.2f} degrees")

        # Apply your empirical formula for the flange's Euler angles
        flange_alpha_cmd_deg = 90.0
        chip_yaw_for_formula = chip_yaw_deg % 360.0 # Normalize to [0, 360) for consistency
        flange_beta_cmd_deg_raw = 180.0 - chip_yaw_for_formula
        flange_beta_cmd_deg = (flange_beta_cmd_deg_raw + 180.0) % 360.0 - 180.0 # Normalize to [-180, 180]
        flange_gamma_cmd_deg = -90.0

        # Convert these target Euler angles into a rotation object for our calculation.
        # CRUCIAL: Ensure 'xyz' matches your robot's Alpha, Beta, Gamma convention.
        try:
            R_flange_target = R.from_euler('xyz', [flange_alpha_cmd_deg, flange_beta_cmd_deg, flange_gamma_cmd_deg], degrees=True)
        except Exception as e:
            self.get_logger().error(f"Failed to create rotation from empirical Euler angles. Check for gimbal lock issues. Error: {e}")
            return
            
        # --- STEP 2: Determine the target POSITION for the FLANGE ---

        # The desired world position for the TCP is the chip's position (plus any approach offset)
        P_tcp_target_m = np.array([chip_pose_in_base_msg.position.x,
                                   chip_pose_in_base_msg.position.y,
                                   chip_pose_in_base_msg.position.z + 0.010]) # Approach 10mm above chip

        # Get the TCP offset vector (from flange frame) from your static TF publisher
        try:
            transform_flange_to_tcp = self.tf_buffer.lookup_transform(
                'meca_axis_6_link', 'tweezer_tcp', rclpy.time.Time(), timeout=Duration(seconds=1.0))
            tcp_offset_in_flange_frame = np.array([transform_flange_to_tcp.transform.translation.x,
                                                   transform_flange_to_tcp.transform.translation.y,
                                                   transform_flange_to_tcp.transform.translation.z])
        except Exception as e:
            self.get_logger().error(f"Could not look up TF from 'meca_axis_6_link' to 'tweezer_tcp': {e}")
            return

        # Calculate the flange position using the consistent orientation
        # P_flange = P_tcp - R_flange * p_tcp_in_flange
        world_offset_from_flange_to_tcp = R_flange_target.apply(tcp_offset_in_flange_frame)
        P_flange_target_m = P_tcp_target_m - world_offset_from_flange_to_tcp

        # --- STEP 3: Assemble the final command and send it ---
        command_pose = [
            1000.0*chip_pose_in_base_msg.position.x+0*P_flange_target_m[0] * 1000.0, # Convert position to mm
            1000.0*chip_pose_in_base_msg.position.y+0*P_flange_target_m[1] * 1000.0,
            100.0,
            flange_alpha_cmd_deg,
            flange_beta_cmd_deg,
            flange_gamma_cmd_deg
        ]

        self.get_logger().info(f"Calculated Target FLANGE Pose (mm, User Euler deg): P({command_pose[0]:.2f}, {command_pose[1]:.2f}, {command_pose[2]:.2f}), O({command_pose[3]:.2f}, {command_pose[4]:.2f}, {command_pose[5]:.2f})")
        current_p = self.current_pose_robot1
        self.get_logger().info(f"Current FLANGE Pose (mm, User Euler deg): P({current_p.position.x:.2f}, {current_p.position.y:.2f}, {current_p.position.z:.2f}), O({current_p.orientation.x:.2f}, {current_p.orientation.y:.2f}, {current_p.orientation.z:.2f}, {current_p.orientation.w:.2f})")
        input("Press Enter here to go to pose...")
        self.get_logger().info(f"Attempting to move to GRASP FLANGE pose...")
        if self.move_pose_and_wait(ROBOT1['namespace'], command_pose[0], command_pose[1], command_pose[2],
                                   command_pose[3], command_pose[4], command_pose[5]):
            self.get_logger().info("Reached target flange pose for grasp.")
        else:
            self.get_logger().error("Failed to reach target flange pose for grasp.")
    ...
    def chip_pose_callback(self, msg: PoseStamped):
        '''
        self.get_logger().info(
            f"Received chip pose in '{msg.header.frame_id}': "
            f"P(x={msg.pose.position.x:.4f}, y={msg.pose.position.y:.4f}, z={msg.pose.position.z:.4f}) "
            f"O(x={msg.pose.orientation.x:.3f}, y={msg.pose.orientation.y:.3f}, z={msg.pose.orientation.z:.3f}, w={msg.pose.orientation.w:.3f})"
        )
        '''
        self.latest_chip_pose_ = msg
